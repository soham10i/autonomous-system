...
class AStar:
    ...
    def find_path(self, start_world, goal_world, robot_radius_grid=0):
        ...
        if not self.grid.is_valid_grid_coord(start_grid[0], start_grid[1]):
            print(f"Start point {start_world} is outside the grid.")
            return None
        if not self.grid.is_valid_grid_coord(goal_grid[0], goal_grid[1]):
            print(f"Goal point {goal_world} is outside the grid.")
            return None

        # Check if start or goal is occupied
        if self.grid.is_occupied(start_world[0], start_world[1]):
            print(f"Start point {start_world} is occupied.")
            return None
        if self.grid.is_occupied(goal_world[0], goal_world[1]):
            print(f"Goal point {goal_world} is occupied.")
            return None
                    ...
                    for r_dx in range(-robot_radius_grid, robot_radius_grid + 1):
                        for r_dy in range(-robot_radius_grid, robot_radius_grid + 1):
                            ...
                            if not self.grid.is_valid_grid_coord(check_x, check_y) or \
                               self.grid.is_occupied(self.grid.grid_to_world(check_x, check_y)[0], 
                                                     self.grid.grid_to_world(check_x, check_y)[1]):
                                is_obstacle = True
                                break
                        if is_obstacle:
                            break
